...
void SurfaceSmoothing::compute_edge_weights(bool use_uniform_laplace)
{
  ...
  {
    for (auto e : mesh_.edges())
      eweight[e] = Scalar(std::max(0.0, cotan_weight(mesh_, e)));
  }

  how_many_edge_weights_ = uint32_t(mesh_.n_edges());
}
...
void SurfaceSmoothing::compute_vertex_weights(bool use_uniform_laplace)
{
  ...
  if (use_uniform_laplace)
  {
    for (auto v : mesh_.vertices())
      vweight[v] = Scalar(1.0 / mesh_.valence(v));
  }
  else
  {
    for (auto v : mesh_.vertices())
      vweight[v] = Scalar(0.5 / voronoi_area(mesh_, v));
  }

  how_many_vertex_weights_ = uint32_t(mesh_.n_vertices());
}

void SurfaceSmoothing::explicit_smoothing(uint32_t iters,

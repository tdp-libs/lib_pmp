                                       ...
                                       Scalar hausdorff_error)
{
    ...
    normal_deviation_ = normal_deviation / Scalar(180.0) * Scalar(M_PI);
    ...
    else
    {
        ...
        for (auto f : mesh_.faces(cd.v0))
        {
            if (f != cd.fl && f != cd.fr)
            {
                ...
                if (nc.angle() > Scalar(0.5) * normal_deviation_)
                {
                    vpoint_[cd.v0] = p0;
                    return false;
                }

...
bool SurfaceParameterization::setup_boundary_constraints()
{
    ...
    for (i = 0, l = 0.0; i < n;)
    {
        ...
        angle = Scalar(2.0) * Scalar(M_PI) * (Scalar(1.0) - l / length);

        t[0] = Scalar(0.5) + Scalar(0.5) * cosf(angle);
        t[1] = Scalar(0.5) + Scalar(0.5) * sinf(angle);
    ...
    for (unsigned int i = 0; i < nv2; ++i)
    {
        ...
        if (!locked[vi])
        {
            ...
            for (auto h : mesh_.halfedges(vi))
            {
                ...
                {
                    b[row] -= sj0 * double(tex[vj][0]);
                    b[row] -= sj1 * double(tex[vj][1]);
                }
            }
